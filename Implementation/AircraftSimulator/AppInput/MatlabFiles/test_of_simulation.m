...
for i = 1:1:length(time)
   ...
   values(i) = result(5);
   if(u_lateral(1) == -max_elevator && index == -1)
       index = i;
   end
   if( i > 10 && i < 150 && mod(i,5) == 0)
       u_lateral = u_lateral + [2 * 2 * pi /360 ; - 2 * 2 * pi /360]; 
   end
...
%    end
   ...
   u_lateral(1) = min(u_lateral(1), max_elevator);
   u_lateral(1) = max(u_lateral(1), -max_elevator);
end
figure(1);
plot(time, values);
hold on;
